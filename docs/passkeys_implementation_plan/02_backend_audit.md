...
|---|---|---|---|---|
| G1 | **Blocker (CONFIRMED 2026-09-02)** | RP id/origins hardcoded `localhost` / `http://localhost:4200`; `AuthConfigProperties.webauthn` is dead config | Native Android Credential Manager rejects `rp.id=localhost` — "RP ID cannot be validated" (verified on emulator). End-to-end passkey registration/login is blocked until a real domain is used | Choose RP domain (recommend apex `fng.rs`); make the RP operations bean read rpId/origins from config (per-profile); host `https://<domain>/.well-known/assetlinks.json` (`get_login_creds` + package `com.vincsoftware.fap_mobile` + SHA-256 fingerprint); app `asset_statements` + iOS AASA to match |
| G2 | **High — backend DONE 2026-09-02** | No endpoint to list current user's passkeys | Settings can't show "is passkey enabled" from server | **Shipped as `GET /v1/account/passkeys`** (Bearer) → `WebAuthnCredential[]` per updated `auth-api.yaml`. Mobile mirror pending (Phase 2-2) |
| G3 | **High — backend DONE 2026-09-02** | No user-scoped delete endpoint | "Remove passkey" can't remove the server credential | **Shipped as `DELETE /webauthn/register/{id}`** (Bearer, owner-scoped) per updated `auth-api.yaml`. Mobile mirror pending (resolves D2) |
