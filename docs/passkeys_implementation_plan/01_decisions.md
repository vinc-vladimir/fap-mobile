...
| D6 | **Native passkey validation requires a real RP domain — `localhost` is not usable.** Android Credential Manager rejects `rp.id=localhost` with "RP ID cannot be validated". To validate passkeys end-to-end (even on the emulator), the backend RP must be a public HTTPS domain serving `assetlinks.json`, with matching app `asset_statements` (+ iOS AASA later). This supersedes D1's "localhost works on the emulator" assumption for the passkey ceremony. Emulator testing remains possible once a real domain is configured. | Confirmed empirically + by `passkeys` plugin docs. | 2026-09-02 | ✅ Confirmed |
